...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        vul_target = df["localization_target"].tolist()
        repair_target = df["target"].tolist()
        for i in tqdm(range(len(source))):
            self.examples.append(convert_examples_to_features(source[i], vul_target[i], repair_target[i], tokenizer, args))
        if file_type == "train":
    ...
    def __getitem__(self, i):       
        return torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].vul_input_ids), torch.tensor(self.examples[i].repair_input_ids)


def convert_examples_to_features(source, vul_target, repair_target, tokenizer, args):
    ...
    vul_input_ids = tokenizer.encode(str(vul_target), truncation=True, max_length=args.vul_query_block_size, padding='max_length')
    repair_input_ids = tokenizer.encode(str(repair_target), truncation=True, max_length=args.vul_repair_block_size, padding='max_length')
    return InputFeatures(input_ids, vul_input_ids, repair_input_ids)
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            loss = model(input_ids=input_ids, attention_mask=input_ids.ne(tokenizer.pad_token_id), labels=repair_input_ids, decoder_attention_mask=repair_input_ids.ne(tokenizer.pad_token_id)).loss
            loss.backward()
...
def main():
    ...
    to_add = ["<S2SV_StartBug>", "<S2SV_EndBug>", "<S2SV_blank>", "<S2SV_ModStart>", "<S2SV_ModEnd>"]
