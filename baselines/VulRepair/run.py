...
from transformers import get_linear_schedule_with_warmup, RobertaTokenizer, T5ForConditionalGeneration
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            loss = model(input_ids=input_ids, attention_mask=input_ids.ne(tokenizer.pad_token_id), labels=repair_input_ids, vul_query_mask=None).loss
            loss.backward()
...
def main():
    ...
    if args.do_eval:
        ...
        model.to(args.device)
        eval_dataset = TextDataset(tokenizer, args, file_type='eval')
        _ =evaluate(args, model, tokenizer, eval_dataset)   
    if args.do_test:
        checkpoint_prefix = f'checkpoint-best-loss/{args.model_name}'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
        model.load_state_dict(torch.load(output_dir, map_location=args.device))
