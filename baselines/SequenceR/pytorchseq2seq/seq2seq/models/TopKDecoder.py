...
class TopKDecoder(torch.nn.Module):
    ...
    def forward(self, inputs=None, encoder_hidden=None, encoder_outputs=None, function=F.log_softmax,
        ...
        sequence_scores = torch.Tensor(batch_size * self.k, 1).cuda()
        sequence_scores.fill_(-float('Inf'))
        sequence_scores.index_fill_(0, torch.LongTensor([i * self.k for i in range(0, batch_size)]).cuda(), 0.0)
        ...
        for _ in range(0, max_length):
            ...
            predecessors = (candidates / self.V + self.pos_index.expand_as(candidates)).view(batch_size * self.k, 1).int().cuda()
    ...
    def _backtrack(self, nw_output, nw_hidden, predecessors, symbols, scores, b, hidden_size):
        ...
        cu_h_n = []
        for h in h_n:
            cu_h_n.append(h.cuda())
        cu_h_n = tuple(cu_h_n)
        if lstm:
            h_t = [tuple([h.index_select(1, re_sorted_idx).view(-1, b, self.k, hidden_size) for h in step]) for step in reversed(h_t)]
            cu_h_n = tuple([h.index_select(1, re_sorted_idx.data).view(-1, b, self.k, hidden_size) for h in cu_h_n])
        else:
        ...
        return output, h_t, cu_h_n, s, l, p
