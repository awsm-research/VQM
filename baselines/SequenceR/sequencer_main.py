...
from sequencer_model import Seq2Seq
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):       
        return torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].repair_input_ids),  torch.tensor(self.examples[i].input_ids).ne(self.tokenizer.pad_token_id), torch.tensor(self.examples[i].repair_input_ids).ne(self.tokenizer.pad_token_id)
...
def convert_examples_to_features(source, repair_target, tokenizer, args):
    ...
    repair_input_ids = tokenizer.encode(repair_target, truncation=True, max_length=args.decoder_block_size, padding='max_length')
    return InputFeatures(input_ids, repair_input_ids)
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            (input_ids, decoder_input_ids, _, tgt_mask) = [x.to(args.device) for x in batch]
            model.train()
            # the forward function automatically creates the correct decoder_input_ids
            output = model(src=input_ids, tgt=decoder_input_ids)

            shift_logits = output.contiguous()
...
def test(args, model, tokenizer, test_dataset):
    ...
    for batch in bar:
        ...
        with torch.no_grad():
            try:
                beam_outputs = model.beam_search(input_ids, decoder_input_ids)
            except:
                count += 1
                continue
...
def main():
    ...
    # build model
    model=Seq2Seq(tokenizer=tokenizer, args=args)
    logger.info("Training/evaluation parameters %s", args)
    # Training
    if args.do_train:
        if args.load_pretrained_model:
            checkpoint_prefix = 'checkpoint-best-loss/pretrained_sequencer.bin'
            output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
