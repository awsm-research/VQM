...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            vul_query = []
            # batch, 512
            for bat in vul_query_label:
                vq = []
                for prob in bat:
                    vq.append([0 for _ in range(768)]) if (prob < 0.5) else vq.append([0.1 for _ in range(768)])
                vul_query.append(vq)
            vul_query = torch.tensor(vul_query, device=args.device)
            #######################                  

            # the forward function automatically creates the correct decoder_input_ids
            loss = model(input_ids=input_ids, vul_query_label=vul_query, repair_input_ids=repair_input_ids)
            loss.backward()
...
def test(args, model, tokenizer, test_dataset):
    ...
    for batch in bar:
        ...
        with torch.no_grad():
            beam_outputs = model(input_ids=input_ids, repair_input_ids=repair_input_ids, vul_query_label=vul_query, generate_repair=True)
