...
def train(args, train_dataset, model, loc_model, tokenizer, eval_dataset):
    """ Train the model """
    # build dataloader
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=len(train_dataloader)*75*0.1, num_training_steps=len(train_dataloader)*75)
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            model.train()
            (input_ids, vul_query_label, repair_input_ids) = [x.to(args.device) for x in batch]
            vul_query_label = loc_model(input_ids)
            # the forward function automatically creates the correct decoder_input_ids
            loss = model(input_ids=input_ids, vul_query_label=vul_query_label, repair_input_ids=repair_input_ids)
            loss.backward()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    eval_loss = evaluate(args, model, loc_model, tokenizer, eval_dataset, eval_when_training=True)    
...
def evaluate(args, model, loc_model, tokenizer, eval_dataset, eval_when_training=False):
    #build dataloader
...
def test(args, model, loc_model, tokenizer, test_dataset):
    ...
    for batch in bar:
        ...
        vul_query_label = loc_model(input_ids)
        with torch.no_grad():
            beam_outputs = model(input_ids=input_ids, repair_input_ids=repair_input_ids, vul_query_label=vul_query_label, generate_repair=True)
...
def main():
    ...
    parser.add_argument("--pretrained_model_name", default="pretrained_model.bin", type=str,
                        help="")
    parser.add_argument("--prob_threshold", default=0.0, type=float,
                        help="")

    parser.add_argument("--evaluate_during_training", action='store_true',
                        help="Run evaluation during training at each logging step.")
    parser.add_argument("--att_adjustment", default=0.0, type=float,
    ...
    if args.load_pretrained_t5:
        t5.load_state_dict(torch.load("saved_models/checkpoint-best-loss/pretrained_model.bin", map_location=args.device))
        print("pretrained T5 loaded!")
    ...
    if args.do_train:
        ...
        train(args, train_dataset, model, loc_model, tokenizer, eval_dataset)
